...
module Paperback::GemfileParser
  ...
  class ParseContext
    ...
    def ruby(version, engine: nil, engine_version: nil)
      req = Paperback::Support::GemRequirement.new(version)
      raise "Running ruby version #{RUBY_VERSION} does not match requested #{version.inspect}" unless req.satisfied_by?(Paperback::Support::GemVersion.new(RUBY_VERSION))
      ...
      if engine_version
        ...
        req = Paperback::Support::GemRequirement.new(version)
        raise "Running ruby engine version #{RUBY_ENGINE_VERSION} does not match requested #{engine_version.inspect}" unless req.satisfied_by?(Paperback::Support::GemVersion.new(RUBY_ENGINE_VERSION))
      end
