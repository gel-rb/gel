...
class Paperback::Support::Tar::TarWriter
  ...
  def add_file(name, mode) # :yields: io
    ...
    raise Paperback::Support::Tar::NonSeekableIO unless @io.respond_to? :pos=
    ...
    header = Paperback::Support::Tar::TarHeader.new :name => name, :mode => mode,
  ...
  def add_file_digest name, mode, digest_algorithms # :yields: io
    ...
    add_file name, mode do |io|
      Paperback::Support::Tar::DigestIO.wrap io, digests do |digest_io|
        yield digest_io
      end
    end
  ...
  def split_name(name) # :nodoc:
    if name.bytesize > 256 then
      raise Paperback::Support::Tar::TooLongFileName.new("File \"#{name}\" has a too long path (should be 256 or less)")
    end
    ...
    if name.bytesize > 100 then
      ...
      if name.bytesize > 100 or prefix.empty? then
        raise Paperback::Support::Tar::TooLongFileName.new("File \"#{prefix}/#{name}\" has a too long name (should be 100 or less)")
      end

      if prefix.bytesize > 155 then
        raise Paperback::Support::Tar::TooLongFileName.new("File \"#{prefix}/#{name}\" has a too long base path (should be 155 or less)")
      end
    end
