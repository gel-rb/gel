...
class ResolveTest < Minitest::Test
  ...
  def gemspec_rz(name:, version:, dependencies: [], ruby: [">=", "0"], platform: "ruby")
    ...
    gem_requirement = lambda { |pairs|
      "U:\x15Gem::Requirement[\x06" +
        array.call(pairs.map { |op, ver| "[\x07" + m.call(op) + gem_version.call(ver) })
    }

    gem_dependency = lambda { |type, dep_name, requirement_pairs|
      "o:\x14Gem::Dependency\n" \
        ":\n@name" + m.call(dep_name) +
        ":\x11@requirement" + gem_requirement.call(requirement_pairs) +
        ":\n@type" + m.call(type) +
        ":\x10@prerelease" + "F" \
        ":\x1A@version_requirements" + gem_requirement.call(requirement_pairs)
    }

    inner = "\x04\b" + array.call([
      m.call("0.0.0"), # @rubygems_version
      m.call(4), # @specification_version
      m.call(name), # @name
      gem_version.call(version), # @version
      m.call(Time.now.utc), # date
      m.call(""), # @summary
      gem_requirement.call([ruby]), # @required_ruby_version
      gem_requirement.call([[">=", "0"]]), # @required_rubygems_version
      m.call(platform), # @original_platform
      array.call(dependencies.map { |type, dep_name, reqs| gem_dependency.call(type, dep_name, reqs) }), # @dependencies
    ])
  ...
  def specs_gz(gems)
    ...
    gz.write(
      "\x04\b" +
      array.call(gems.map { |name, version, platform|
        array.call([m.call(name), gem_version.call(version), m.call(platform)])
      })
    )
    gz.close
